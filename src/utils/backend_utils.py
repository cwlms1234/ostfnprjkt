...
def toggle_pump(cfg: dict, data: dict) -> None:
    ...
    if data[reading_col] >= cfg["temperature_thresholds"]["activation_threshold"]:
        print(f"*** Measured {data[reading_col]} >= {cfg["temperature_thresholds"]["activation_threshold"]} \n Powering pump")
    elif data [reading_col] <= cfg["temperature_thresholds"]["activation_threshold"]:
        print(f"*** Measured {data[reading_col]} >= {cfg["temperature_thresholds"]["deactivation_threshold"]} \n Shutting down pump")
    else:
        pass
